...
class SequenceExecutor(private val plugin: KantanCommanderPlugin) {
        ...
        when (node.type) {
            ...
            CommandType.CONTINUE -> continueFor(script, graph, node, session, depth, done)
                ...
                val outcome = when {
                    ...
                    executeImmediate(node, session) -> {
                        if (session.lastDetail.isNullOrBlank()) {
                            session.lastDetail = immediateDetail(node.type)
                        }
    ...
    private fun executeImmediate(node: CommandNode, session: ExecutionSession): Boolean = runCatching {
        ...
        val failure = it.message?.takeIf(String::isNotBlank) ?: it::class.simpleName ?: "command_failed"
        session.lastFailureReason = failure
        session.lastDetail = "${immediateDetail(node.type)}（実行に失敗: $failure）"
