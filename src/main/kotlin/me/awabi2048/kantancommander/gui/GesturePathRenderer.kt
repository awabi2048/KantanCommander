 ...
 * 接続ごとに独立した3枚の帯へ分割します。画面外のセルも呼び出し側が余白付きで
 * 渡すため、画面端で切り取られる経路も同じ計算規則で描画されます。
 */
internal object GesturePathRenderer {
    private val pathKinds = setOf(
        MapCellKind.PATH,
        MapCellKind.BRANCH_PATH,
        MapCellKind.LOOP_RETURN_PATH,
    )
    private val connectableKinds = pathKinds + setOf(MapCellKind.NODE, MapCellKind.ADD)
    ...
    fun buildSegments(
        ...
        val points = cells.keys.sortedWith(compareBy<MapPoint>({ it.y }, { it.x }))
        val adjacency = points.associateWith { point ->
            directions.map { direction -> MapPoint(point.x + direction.x, point.y + direction.y) }
                .filter { neighbor -> cells[neighbor]?.kind in connectableKinds }
        }
        val degree = adjacency.mapValues { (_, neighbors) -> neighbors.size }
        fun isTerminal(point: MapPoint): Boolean =
            cells[point]?.kind in setOf(MapCellKind.NODE, MapCellKind.ADD) || degree[point] != 2
        ...
        fun isJunction(point: MapPoint, segmentAxis: Axis): Boolean {
            if (cells[point]?.kind !in pathKinds) return false
        ...
        points.filter { point ->
            if (cells[point]?.kind !in pathKinds) return@filter false
