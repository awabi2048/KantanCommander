...
class GraphLayoutEngineTest {
    ...
    fun `for start body end and return path are rendered as one structure`() {
        ...
        val returnY = layout.cells.keys.filter {
            ...
            .minOf(MapPoint::y)
        assertEquals(
            setOf(MapPoint(startPoint.x + 1, returnY), MapPoint(startPoint.x + 3, returnY)),
            layout.loopReturnArrowPoints,
        )
        assertTrue(layout.loopReturnArrowPoints.none { it.x == startPoint.x || it.x == endPoint.x })
    ...
    @Test
    fun `loop return arrows skip boundaries and every other return path slot inside branches`() {
        val graph = CommandGraph.empty()
        val start = GraphEditor.append(graph, CommandType.FOR_START)
        ...
        val returnY = layout.cells.keys.filter {
            it.x == startPoint.x + 1 &&
                it.y > startPoint.y &&
                layout.cells[it]?.kind == MapCellKind.LOOP_RETURN_PATH
        }
            .minOf(MapPoint::y)
        val expectedX = (startPoint.x + 1 until endPoint.x step 2).toSet()
    ...
    @Test
    fun `empty loop return arrows use the same boundary-free stride`() {
        val graph = CommandGraph.empty()
        val start = GraphEditor.append(graph, CommandType.FOR_START)
        val layout = GraphLayoutEngine.layout(graph)
        val startPoint = requireNotNull(layout.nodePoints[start.id])
        ...
        assertEquals(
            (startPoint.x + 1 until endPoint.x step 2)
                .map { MapPoint(it, returnY) }
                .toSet(),
            layout.loopReturnArrowPoints,
        )
        assertTrue(layout.loopReturnArrowPoints.all { layout.cells[it]?.kind == MapCellKind.LOOP_RETURN_PATH })
